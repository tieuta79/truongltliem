...
class ConfigNetwork {
    ...
    public function __construct() {
        if (!Plugin::loaded('Sites')) {
            Plugin::load('Sites', ['bootstrap' => true, 'routes' => true]);
        }
    }

    public function configOtherDb() {
    ...
    public function setDbNetwork() {
        ...
        if (Configure::read('Network.type') == 1) {
            ...
            if ($network) {
                $db = $configDefault['database'] . '_' . substr($network, 1);
            }
        } else {
            $domainMapping = Configure::read('Network.domainMapping');
            if (isset($domainMapping[$request->domain()])) {
                $db = $configDefault['database'] . '_' . $domainMapping[$request->domain()];
            } else {
                if ($request->subdomains()) {
                    $db = $configDefault['database'] . '_' . $request->subdomains()[0];
                }
            }
        }
    ...
    public function getCurrentDb() {
        $configDefault = ConnectionManager::get('default')->config();
        $db = $this->getCurrentScope();
        if (!empty($db)) {
            return $configDefault['database'] . '_' . $db;
        }
        return 'default';
    }
    ...
    public function getCurrentScope() {
        ...
        $network = Network::checkScopeByUrl($request->here, $this->domains);
        if ($network) {
            $scope = substr($network, 1);
        }
        return $scope;
    }

    public function getDataMigrations($modal) {
