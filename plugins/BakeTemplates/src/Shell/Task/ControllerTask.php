 ...
 *
 */
class ControllerTask extends SimpleBakeTask {
    ...
    public function bake($controllerName) {
        ...
        if (empty($this->params['no-actions'])) {
            $actions = ['beforeFilter', 'index', 'view', 'add', 'edit', 'delete'];

            $event = EventManager::instance()->dispatch(new Event('Base.setActions', ['model' => $controllerName, 'prefix' => $this->_getPrefix(), 'scaffoldActions' => $actions, 'noTemplateActions' => []]));
            if (!empty($event->result)) {
                $actions = $event->result['scaffoldActions'];
            }
        }
        ...
        $associations = $this->Model->findBelongsTo($modelObj,$associations);
        $associations = $this->Model->findHasMany($modelObj,$associations);
        $associations = $this->Model->findBelongsToMany($modelObj,$associations);
        
        if(count($associations['belongsToMany'])>0){
            $actions[] = 'addRelation';
            $actions[] = 'editRelation';
            $actions[] = 'deleteRelation';
            
            $actions[] = 'addAssociated';
            $actions[] = 'editAssociated';
            $actions[] = 'deleteAssociated';            
        }
