...
namespace BakeTemplates\Shell\Task;
 ...
 *
 */
class TemplateTask extends SimpleBakeTask {
     ...
     * @var array
     */
    public $scaffoldActions = ['beforeFilter','index', 'view', 'add', 'edit'];
     ...
     * @var array
     */
    public $noTemplateActions = ['beforeFilter','delete','deleteRelation','deleteAssociated'];
    ...
    public function main($name = null, $template = null, $action = null)
    {
        ...
        $event = EventManager::instance()->dispatch(new Event('Base.setActions', ['model'=>$name,'prefix'=>$this->_getPrefix(),'scaffoldActions'=>$this->scaffoldActions,'noTemplateActions'=>$this->noTemplateActions]));
        if(!empty($event->result)){
            $this->scaffoldActions = $event->result['scaffoldActions'];
            $this->noTemplateActions = $event->result['noTemplateActions'];
        }
    ...
    public function generateHTML() {
        return $this->param('generateHTML')=='true'?true:false;
    }
    ...
    public function getContent($action, $vars = null)
    {
        ...
        $path_bake = 'Template';
        if($this->generateHTML() && $this->param('prefix')=='admin'){
            $path_bake = 'Template/html';
        }
        
        $path_template = \Cake\Core\App::path('Template/Bake/'.$path_bake,'BakeTemplates')[0];
        if(!file_exists($path_template.$action.'.ctp')){
            return 'template for action '.$action;
        }
        
        return $this->BakeTemplate->generate("$path_bake/$action");
    }
    ...
    public function getOptionParser()
    {
        ...
        ])->addOption('generateHTML', [
            'help' => 'Generate views all tags for.',
            'short' => 'g',
        ])->addSubcommand('all', [
