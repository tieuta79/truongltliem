 ...
 */
class ModelTask extends SimpleBakeTask
{
    /**
    ...
    public function bake($name)
    {
        ...
        $hidden = $this->getHiddenFields($model);
        ...
        //$this->generalMenusAdmin($model->alias());
    }
    
    public function generalMenusAdmin($name) {
        Configure::load('Cms.menu', 'default');        
        $menus['Menus']['Admin'] = Configure::read('Menus.Admin');
        if(!isset($menus['Menus']['Admin'][$name])){
            $menus['Menus']['Admin'][$name] = [
                'icon' => 'fa fa-circle-o',
                'title' => $name,
                'url' => '#',                
                'child' => [
                ...
                'priority' => (count($menus['Menus']['Admin'])+1)            
            ];
        }
        $path_menu = \Cake\Core\Plugin::path('Cms').'config'.DS.'menu.php';
        if(file_exists($path_menu)){
            $this->_deleteEmptyFile($path_menu);
        }
        $menus_tring = "<?php\nreturn [".$this->stringifyList($menus)."];";
        //pr($menus_tring);die();
        $this->createFile($path_menu, $menus_tring);
    }
    ...
    public function stringifyList($list, $options = []) {
        ...
        foreach ($list as $k => &$v) {
            if(is_array($v)){         
                $opts = $options;
                $opts['indent'] = intval($opts['indent'])+$i;
                if (!is_numeric($k)) {                    
                    $result .= $start."'$k' => [".$this->stringifyList($v,$opts)."],".$end;
                }  else {
                    $result .= $start."$k => [".$this->stringifyList($v,$opts)."],".$end;
                }
        ...
        //$result .= $start . implode($join, $list) . $end;
        return $result;
    }
    ...
    public function bakeTable($model, array $data = [])
    {
        ...
        $it_fields = $this->getEntityPropertySchema($model);
