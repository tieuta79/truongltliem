...
EventManager::instance()->on('Admin.Tables.' . $joinTable . '.rowAction', function (Event $event) use ($assocSingularTable){
    $subject = $event->subject();
    $action = $subject['action'];
    $action['Edit'] = $subject['helper']->Html->link(
            '<i class="fa fa-pencil-square-o"></i>', ['controller' => $this->request->controller, 'action' => 'editRelation', $this->request->params['pass'][0], $this->request->params['pass'][1], $subject['row']->{$assocSingularTable}->id], ['escape' => false, 'class' => 'btn btn-success btn-sm', 'data-toggle' => 'tooltip', 'title' => __d('ittvn', 'Edit')]
    );
    return $action;
});
...
$this->Html->addCrumb(__d('ittvn', 'Edit <%= ucfirst($singularVar) %>'), ['controller' => '<%= $modelClass %>', 'action' => 'edit', $assocId]);
$this->Html->addCrumb(__d('ittvn', 'Add ' . $assocTable), $this->request->here);

echo $this->cell('Cms.Forms::relation', ['data'=>$this->viewVars]);
?>
<% }else{ %>
<?php
use Cake\Utility\Inflector;
