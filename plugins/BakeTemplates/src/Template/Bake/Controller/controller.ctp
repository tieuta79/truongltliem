...
$path = \Cake\Core\App::path('Template/Bake/Element/Controller','BakeTemplates')[0];
$path_template = \Cake\Core\App::path('Template/Bake/Template','BakeTemplates')[0];

foreach($actions as $action) {    
    if(!file_exists($path . $action.'.ctp')){
        echo $this->element('Controller/action',['currentAction'=>$action]); 
    }else{
