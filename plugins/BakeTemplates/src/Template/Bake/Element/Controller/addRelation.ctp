...
%>
     ...
     * @return void.
     */
    public function addRelation($assocTable, $id)
    {
        ...
        if(count($associations['belongsToMany'])>0){            
            ...
            ${$assocVariable} = $this-><%= $currentModelName %>->{$assocModel}->newEntity();
            ...
            if ($this->request->is('post')) {
                if (!empty($this->request->data[$this-><%= $currentModelName %>->{$assocModel}->displayField()])) {
                    ...
                    if ($this-><%= $currentModelName %>->{$assocModel}->save(${$assocVariable})) {
                        $this->loadModel($joinTableModel);
                        ${$joinTableVariable} = $this->{$joinTableModel}->newEntity([
                            $foreignKey=>$this->request->data[$foreignKey],
                            $targetForeignKey=> ${$assocVariable}->id
                        ]);
                        $this->$joinTableModel->save(${$joinTableVariable});               
                        $this->Flash->success(sprintf(__('The %s has been saved.'),$assocVariable));

                        return $this->redirect(['action' => 'addRelation',$assocTable,$id]);
                    } else {
