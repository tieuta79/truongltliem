...
%>
     ...
     */
    public function deleteRelation($assocTable, $id, $relation_id)
    {
        ...
        ${$join_table} = $this->{$join_table}->find()->where(['id'=>$relation_id])->first();        
        if ($this->{$join_table}->delete(${$join_table})) {
