...
%>
     ...
     * @return void.
     */
    public function addAssociated($assocTable, $joinTable, $id)
    {
        $this->loadModel($this->request->plugin . '.' . $joinTable);
        ${$joinTable} = $this->{$joinTable}->newEntity();
        if ($this->request->is('post')) {
            if (!empty($this->request->data['<%= $singularName; %>_id'])) {
                ...
                if ($this->{$joinTable}->save(${$joinTable})) {
                    ...
                    return $this->redirect(['action' => 'addAssociated', $assocTable, $joinTable, $id]);
                } else {
                    ...
                    $query->contain(['<%= $currentModelName %>', $assocTable])->where(['<%= $singularName; %>_id' => $id]);
