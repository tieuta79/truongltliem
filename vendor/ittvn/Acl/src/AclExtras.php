...
class AclExtras {
    ...
    public function acoUpdate($params = []) {
        ...
        if (empty($params['plugin'])) {
            ...
            if (!Plugin::loaded($plugin)) {
                $this->err(__d('ittvn', "<error>Plugin {0} not found or not activated.</error>", [$plugin]));
                return false;
            }
        ...
        $this->out(__d('ittvn', '<success>Aco Update Complete</success>'));
        return true;
    }
    ...
    protected function _checkNode($path, $alias, $parentId = null) {
        if(Configure::check('Site.buildPermission')){
            $connection = Configure::read('Site.buildPermission');
        }else{
            $connection = Configure::read('Network.db');
        }
        $this->Aco->connection(ConnectionManager::get($connection));
        ...
        if (!$node) {
            ...
            $this->out(__d('ittvn', 'Created Aco node: <success>{0}</success>', $path));
        } else {
            $node = $node->first();
        }
        return $node;
    ...
    protected function _checkMethods($className, $controllerName, $node, $pluginPath = null, $prefixPath = null) {
        ...
        if ($methods == null) {
            $this->err(__d('ittvn', 'Unable to get methods for {0}', $className));
            return false;
        }
    ...
    protected function _buildPrefixes() {
        $system = new System();
        $plugins = $system->plugins(false, 'system');
        $devplugins = $system->listPlugin();
        $devplugins = array_keys(Hash::extract($devplugins, 'active'));
        $plugins = Hash::merge($plugins, $devplugins);
        ...
        if (count($list_prefix) > 0) {
            foreach ($list_prefix as $lp) {
                if (!empty($lp)) {
                    foreach ($plugins as $plugin) {
                        $this->pluginPrefixes[$plugin][$lp] = true;
                    }
                }
            }
        }
        ...
        foreach ($nodes as $node) {
            if (!isset($methodFlip[$node->alias])) {
                ...
                if ($this->Aco->delete($entity)) {
                    $this->out(__d('ittvn', 'Deleted Aco node: <warning>{0}</warning> and all children', $path));
                }
            }
        }
    }
