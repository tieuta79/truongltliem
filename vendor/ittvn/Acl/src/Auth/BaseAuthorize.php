...
abstract class BaseAuthorize extends ParentAuthorize
{
     ...
     */
    public function action(Request $request, $path = '/:plugin/:prefix/:controller/:action/:params')
    {
		...
		$params = (isset($request['pass'][0]) && !empty($request['pass'][0])) ? $request['pass'][0]: null;
        $path = str_replace(
            [':controller', ':params',':action', ':plugin/', ':prefix/'],
            [Inflector::camelize($request['controller']),$params, $request['action'], $plugin, $prefix],
