                        ...
                        <div class="ibox-content">
                            ...
                            <?php if ($i++ == 0): ?>
                                ...
                                <div class="form-group">
                                    ...
                                    <?php
                                    echo $this->Html->link(
                                            '<i class="fa fa fa-undo"></i> ' . __d('ittvn', 'Back'), ['plugin' => $this->request->plugin, 'controller' => $this->request->controller, 'action' => 'edit', $this->request->params['pass'][1]], ['class' => 'btn btn-info btn-sm', 'data-toggle' => 'tooltip', 'title' => 'Back', 'escape' => false]
                                    );
...
if (count($belongsToMany) > 0):
    ...
    foreach ($belongsToMany as $bTMany):
        ...
        $columns = $bTMany['schema']->columns();
        $belongsTo = [];

        $headers = [
            $bTMany['foreignKey'] => [
                'label' => $modSingularize . ' ' . $displayField,
        ...
        foreach ($columns as $column) {
            if (!in_array($column, array('id', 'delete'))) {
                if (strpos('_id', $column)) {
                    $headers[$column] = [
                        'label' => Inflector::underscore($singularize_alias),
                        'map' => [Inflector::underscore($singularize_alias), $column],
                        'filter' => 'list'
                    ];
                } else {
                    $headers[$underscore_alias.'___'.$column] = [
                        'label' => Inflector::humanize($column),
                        'map' => [$underscore_alias, $underscore_alias.'___'.$column],
                        'filter' => 'text'
                    ];
                }
            }
